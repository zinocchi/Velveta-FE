      ...
      try {
        ...
        const ordersWithImages = ordersData.map((order: Order) => ({
          ...
          items: order.items.map((item: OrderItem) => ({
            ...item,
            image_url: item.menu?.image_url || getItemImage(item.menu_id, item.menu?.name),
          })),
        }));
                ...
                <div class="items-with-images">
                  ${selectedOrder.items
                    .map(
                      (item) => `
                    <div class="item-row">
                      ${item.menu?.image_url ? `<img src="${item.menu.image_url}" alt="${item.menu.name}" class="item-image" />` : '<div style="width:40px;height:40px;background:#f0f0f0;border-radius:4px;display:flex;align-items:center;justify-content:center;">📦</div>'}
                      ...
                      {selectedOrder.items.map((item) => (
                        <div key={item.id} className="flex items-center gap-3">
                          ...
                          <div className="w-16 h-16 bg-gradient-to-br from-red-100 to-red-50 rounded-lg flex items-center justify-center shadow-sm overflow-hidden flex-shrink-0">
                            ...
                            <div
                              className={`${item.menu?.image_url ? "hidden" : ""} fallback w-full h-full flex items-center justify-center`}
                            >
