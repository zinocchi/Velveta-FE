// src/pages/menu/components/MenuSidebar.tsx
    ...
    <>
              ...
              {DRINK_CATEGORIES.map((cat) => (
                <li key={cat.id}>
                  <NavLink to={`/category/${cat.id}`} className={linkClass}>
                    <span className="text-sm md:text-base">{cat.displayName}</span>
                  </NavLink>
                </li>
              ))}
            </ul>
      ...
      <div className="lg:hidden mb-6">
        <div className="flex space-x-2 overflow-x-auto pb-2 scrollbar-hide">
          ...
          {[...DRINK_CATEGORIES.slice(0, 3), ...FOOD_CATEGORIES.slice(0, 2)].map((cat) => (
            <NavLink
              key={cat.id}
              to={`/category/${cat.id}`}
              className={({ isActive }) =>
