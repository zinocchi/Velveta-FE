...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
  ...
  const getButtonColor = () => {
    if (newStatus === 'CANCELLED') return 'bg-red-600 hover:bg-red-700';
    if (newStatus === 'COMPLETED') return 'bg-green-600 hover:bg-green-700';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-fadeIn">
        ...
        <h3 className="text-xl font-bold text-gray-900 text-center mb-2">Confirm Status Update</h3>

        <p className="text-gray-600 text-center mb-6">
          {type === 'single' ? (
            <>
              Are you sure you want to change order #{orderNumber}<br />
              status to <span className={`px-2 py-0.5 rounded-full text-sm ${config.bgColor} ${config.color}`}>
                {newStatus}
              </span>?
            </>
          ) : (
            <>
              Are you sure you want to change <span className="font-semibold">{selectedCount}</span> selected orders<br />
              status to <span className={`px-2 py-0.5 rounded-full text-sm ${config.bgColor} ${config.color}`}>
                {newStatus}
              </span>?
            </>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={onConfirm}
            disabled={updating}
            className={`flex-1 px-4 py-3 rounded-xl text-white font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2 ${getButtonColor()}`}
          >
            {updating ? (
              <>
