...
const MenuSidebar = ({ activeCategory, onCategoryClick }: MenuSidebarProps) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = [...drinkCategories, ...foodCategories].map(
        (cat) => cat.id
      );
      let current = "";
  ...
  return (
    <aside className="w-full md:w-64 flex-shrink-0 mb-8 md:mb-0 md:mr-8">
      ...
      `}
      >
        <h2 className="font-bold text-xl text-gray-900 mb-6 font-montserrat border-b pb-3">
          Categories
        </h2>

        {/* Drink Categories */}
        <div className="mb-8">
          <h3 className="text-lg font-semibold text-gray-800 mb-3 font-montserrat">
            Drinks
          </h3>
        ...
        <div className="md:hidden mt-8 pt-6 border-t border-gray-200">
          <div className="flex overflow-x-auto pb-2 space-x-2">
            {[...drinkCategories, ...foodCategories]
              .slice(0, 6)
              .map((category) => (
                <button
                  key={category.id}
